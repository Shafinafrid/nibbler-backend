...
from app.services.profile_resolution import normalize_profile_name, profile_display_name
...
# Retained for wire compatibility; stored names are protected with or without it.
...
def merge_growth_state(stored_state: dict, incoming_state: dict, *,
    ...
    for pid, stored_body in stored_profiles.items():
        ...
        # A root update is not evidence that this profile body changed.
        ...
        if incoming_at is None and stored_at is None:
            ...
            # stored body holds.
            winner = stored_body
            logger.warning("growth merge retained %s: missing or invalid profile timestamp", pid)
        ...
        if winner is incoming_body:
            ...
            # Existing rows predating _canonicalName are authoritative too.
            name = stored_body.get("profileName") or stored_body.get("name")
            canonical = {"id": pid, "name": name, "profileName": name}
            changed = {k: v for k, v in canonical.items() if incoming_body.get(k) != v}
            ...
            merged[pid] = stored_body
            changed = {field: stored_body.get(field) for field in CANONICAL_PROFILE_FIELDS
                       if field in stored_body and incoming_body.get(field) != stored_body.get(field)}
            if changed:
                canonical_fields.append({"id": pid, "fields": changed})
    ...
    used_names = {normalize_profile_name(profile_display_name(p)) for p in merged.values()}
    for pid, incoming_body in incoming_profiles.items():
        if pid in merged:
            continue
        name_key = normalize_profile_name(profile_display_name(incoming_body))
    ...
    # Preserve the plan's explicit compatibility exception for completely
    # unstamped old clients, but never lend a partially stamped push a clock.
    legacy_unstamped = "updatedAt" not in incoming_state and all(
        "updatedAt" not in p for p in incoming_profiles.values()
    )
    if "person" in incoming_state:
        if legacy_unstamped:
            result["person"] = incoming_state.get("person")
        elif incoming_root_at is None and stored_root_at is not None:
            logger.warning("growth merge retained person: missing or invalid root timestamp")
        elif not stored_root_at or (incoming_root_at and incoming_root_at >= stored_root_at):
            result["person"] = incoming_state.get("person")
