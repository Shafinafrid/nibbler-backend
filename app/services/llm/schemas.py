...
# not documentation.
#
# "personalize" (Aug 2026): the dynamic growth-profile question card. Unlike
# every other kind, its content is never generated inline by the deck call —
# it's pre-generated by generate_personalization_question, validated for
# grounding on its own (validation.py's validate_personalization), and
# handed to the deck prompt to reproduce verbatim at a pinned position (see
# session_service._roll_personalization). It only appears in CARD_KINDS so
# the deck-generation schema/validation can recognise and place it.
CARD_KINDS = ["hook", "insight", "quiz", "prompt", "summary", "personalize"]
...
def card_schema(with_images: bool = False) -> Dict[str, Any]:
    ...
    props: Dict[str, Any] = {
        ...
        "explanation": _str(nullable=True),
        # Personalize cards only. A SEPARATE field from `options` above,
        # deliberately not a reuse: quiz's `options` is locked to exactly 4
        # items each carrying a meaningless `correct: false` placeholder for
        # a preference choice, which would force the app to know to ignore
        # it. personalizeOptions instead carries {text, tag} pairs sized
        # 2-4 — see personalization_option_schema.
        "personalizeOptions": {
            "type": ["array", "null"],
            "items": personalization_option_schema(),
            "minItems": 2,
    ...
    required = [
        "kind", "eyebrow", "title", "body", "highlight", "options", "explanation",
        "personalizeOptions",
    ]
    if with_images:
