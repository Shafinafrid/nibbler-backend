...
- **Nibble pictures were removed (2026-09-06).** No AI-image generation, book-figure extraction, selection, image prompt/schema, or card renderer remains. Older clients' image endpoint returns 410 without contacting S3. Stored session responses strip legacy image fields. Book covers/avatars and scanned-document OCR are unrelated and remain.
- Keep ownership-scoped legacy image deletion and its cleanup ledger: already-stored objects still require book/account erasure. No production object purge or destructive column migration is part of this removal.
- **Question novelty (2026-09-06):** the server reads previous answered AND pending questions across all books for the assigned profile. Already-asked tag dimensions are excluded, prompt history supplies prior answers, and a final user-scoped transactional check rejects concurrent repeats. When the existing vocabulary has no unexplored dimension, omit the optional question instead of re-confirming a known answer.
...
- Free: 3 uploads, 1 new session/day, 7-day history; Connect goal match and reading statistics are available on the user's unlocked books. Connect chat and the goal passage remain Premium-only. Premium: 3 sessions/day, ≤5 active sources (`PATCH /library/{id}/active`).
